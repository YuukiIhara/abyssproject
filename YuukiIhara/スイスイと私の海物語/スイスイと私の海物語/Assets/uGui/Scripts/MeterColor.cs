...
public class MeterColor : MonoBehaviour {
	...
	void Update () 
	{
		...
		if (GameObject.Find("Meter").GetComponent<MeterController>().GetHit() == false && bMoveFlag == false) 
		{
			fAlpha = cfAlpha;
			nRed = 255;
			nGreen = 255;
		...
		if (bMoveFlag == true) 
		{
			...
			if(fAlpha <= -2.0f)
			{
				...
				// ポジションの初期化
				Addnum = GameObject.Find("Meter").GetComponent<MeterController>().SetMeterPosition(rot/10);
				rectTransform.position = Addnum;
			}
		}
	}
